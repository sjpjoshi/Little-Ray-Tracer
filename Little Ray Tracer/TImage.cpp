...
LRT::Texture::TImage::TImage() { } // TImage

LRT::Texture::TImage::~TImage() {
	if (m_imageLoaded) {
...
qbVector<double> LRT::Texture::TImage::getColor(const qbVector<double>& uvCoords) {
...
bool LRT::Texture::TImage::loadImage(std::string fileName) {
	if (m_imageLoaded) 
		SDL_FreeSurface(m_imageSurface); 
